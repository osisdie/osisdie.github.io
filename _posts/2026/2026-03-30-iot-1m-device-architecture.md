...
> **English Abstract** — A phased IoT architecture for 1M concurrent devices. **Core architecture**: **EMQX** MQTT v5 broker (Rule Engine direct write) → **TimescaleDB** (hot 7d + Continuous Aggregates) → **FastAPI** backend → **BFF** (Backend for Frontend, WebSocket management + API aggregation) → Dashboard. Security via **mTLS X.509** with software-based certificate rotation, multi-tenant isolation via MQTT topic ACLs + PostgreSQL RLS, and **OpenTelemetry** observability. **Scale-out** (>1M devices): add Redpanda event streaming, ClickHouse OLAP, Flink CEP, multi-region DR. Includes cost estimation (~$22-43K/month), 4-phase rollout plan, and simplified alternatives for smaller deployments. All services are cloud-neutral with AWS/GCP mapping.
...
EMQX Rule Engine 直接寫入 TimescaleDB，不經 Event Streaming 層。架構簡潔、延遲低（<50ms），適合 1M 以下設備規模。超過 1M 設備且需要 dedup、event replay、多消費者場景時，再加入 Redpanda（見 Scale-Out 章節）。
...
```
Device → MQTT → EMQX (Rule Engine) → TimescaleDB → FastAPI → BFF → WebSocket → Dashboard
```

EMQX Rule Engine 直接寫入 TimescaleDB，無需中間 Event Streaming 層。BFF 管理所有 WebSocket 連線，Backend 不直接面對瀏覽器。多個 BFF instance 間用 Redis Pub/Sub 同步。
...
核心架構不使用獨立的 Event Streaming 層。EMQX 內建的 **Rule Engine** 直接將 MQTT 訊息寫入 TimescaleDB，減少組件數和運維複雜度：

```
EMQX Rule Engine 支援：
  - SQL-like 過濾：SELECT * FROM "telemetry/#" WHERE payload.temperature > 50
  - 直接寫入 TimescaleDB（內建 PostgreSQL connector）
  - 訊息轉發、格式轉換、基本 dedup
  - 內建 Rate Limiting + 背壓控制
```

**何時需要獨立 Event Streaming（>1M 設備）：** 當需要 event replay、多消費者（同一份資料寫入多個系統）、複雜 dedup、或跨設備關聯分析時，再加入 Redpanda / Kafka + FastStream / Benthos（見 Scale-Out 章節）。
...
flowchart LR
    ...
    E -->|Rule Engine| H[TimescaleDB<br/>Hot 7 days]
...
| Streaming (>1M) | Redpanda Cloud / MSK | Redpanda Cloud | Redpanda on K8s |
...
flowchart LR
    ...
    E -->|Rule Engine filter| T[TimescaleDB<br/>Write Layer]
    T -->|circuit breaker| A[FastAPI<br/>App Layer]

    E -->|超過 10x| X1[Disconnect]
    T -->|write fail| X2[Alert + Backoff]
    A -->|anomaly detect| X3[Alert]
```

| 層 | 限制 | 超限動作 |
|---|---|---|
| **EMQX** | Per-client 10 msg/s, 50 KB/s | Throttle → disconnect |
| **Rule Engine** | SQL filter + dedup | 丟棄不符合條件的訊息 |
| **FastAPI** | Per-tenant rate limit, anomaly detection | Alert + reject |
...
| Rule Engine | SQL WHERE 過濾 + timestamp 比對 |
...
flowchart LR
    ...
    A[FastAPI] -->|traces + metrics| OT[OpenTelemetry Collector]
    BF[BFF] -->|traces + metrics| OT
...
| 組件 | 監控重點 | 告警閾值 |
|---|---|---|
| EMQX | 連線數、msg rate、ACL deny、Rule Engine throughput | 連線 > 900K、deny rate > 1% |
| TimescaleDB | Write throughput、disk usage、query latency | Write < 80K/s、disk > 80% |
| FastAPI | Request latency、error rate | P99 > 200ms、error > 0.1% |
| BFF | WebSocket connections、aggregation latency | WS > 10K、P99 > 100ms |
...
| K8s (Backend + BFF) | 3-5 nodes | ~$2-4K | ~$2-4K |
| Observability | Grafana Cloud / self-hosted | ~$1-3K | ~$1-3K |
| **合計** | | **~$17-33K** | **~$17-33K** |
| + Redpanda (>1M) | 3 node, 100K msg/s | +$5-10K | +$5-10K |
...
| **10-100 萬** | EMQX + TimescaleDB + ClickHouse + FastAPI + BFF | — | ~$10-20K |
| **> 100 萬** | + Redpanda + FastStream + Multi-Region | 加 event streaming + Flink CEP | ~$30-60K |
...
| 風險 | 說明 | 緩解措施 |
|---|---|---|
| 運維複雜度 | 核心 4 個系統（EMQX + TimescaleDB + FastAPI + BFF），scale-out 加 Redpanda + ClickHouse | OpenTelemetry Day 1、統一 Grafana dashboard |
...
```
Phase 1 (Month 1-2): EMQX + TimescaleDB + FastAPI + BFF + OTel（核心 MVP）
Phase 2 (Month 3-4): + ClickHouse + S3 archival（冷熱分層）
Phase 3 (Month 5-6): + Multi-tenant + DR
Phase 4 (>1M 時):    + Redpanda + FastStream / Benthos（event streaming）
```
...
| Data Ingestion | **EMQX Rule Engine** | — | — | 直寫 DB，無需中間層 |
| Streaming (>1M) | **Redpanda** | Redpanda Cloud / MSK | Redpanda Cloud | Scale-out 時加入 |
