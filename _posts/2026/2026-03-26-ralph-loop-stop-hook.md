...
追查後才發現，這是安裝 [claude-plugins-official](https://github.com/anthropics/claude-plugins-official) marketplace 時一起帶入的 plugin。腳本沒有執行權限（`chmod +x`），所以每次都報 Permission Denied。修正權限後錯誤消失，但也因此讓我深入研究了這個設計精巧的 Stop Hook。
