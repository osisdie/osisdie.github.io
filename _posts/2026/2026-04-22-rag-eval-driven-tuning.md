---
layout: post
title: "RAG Pipeline Eval-Driven Tuning：Threshold Sweep、Progressive Fallback 與 Date Hint 的實證收斂"
...
本帖用三組變更把這三個 open item 收掉，並讓每個收斂都留下可觀察、可回歸的訊號。

---

## 為什麼 Magic Number 必須 Eval-Driven 收斂
...
Eval-driven 收斂的核心是把每個 magic number 關聯到**一組可重跑的量測**：golden set、threshold sweep、per-tier 命中 histogram、schema validator。任何人想調整，都得先展示新的數據；任何 regression，也會在下一次 sweep 立即顯現。

今天這三個 open item 各自對應一種收斂路徑：

| Open Item | 預設值 | 錯誤成本 | 收斂方式 |
| --- | --- | --- | --- |
...
延伸閱讀：[Production-Ready RAG Pipeline]({% post_url 2026-04-18-rag-ingestion-lessons-learned %}) — 本帖收斂的三個 open item 出處；[FAQ Hybrid Search RAG Pipeline 實戰]({% post_url 2026-04-16-faq-hybrid-search-rag %}) — dense + sparse + RRF + dense cosine confidence 的原始骨架。
